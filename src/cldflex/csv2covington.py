...
def replace_reduplication(text):
    ...
    text = text.replace('INF', 'INF\\textasciitilde ')
    text = text.replace('ADV', 'ADV\\textasciitilde ')
    text = text.replace('DIM', 'DIM\\textasciitilde ')
...
def convert_leipzig(gloss):
    ...
    gloss = gloss.replace('PN', '{\\Pn}')
    gloss = gloss.replace('PRIV', '{\\Priv}')
    gloss = gloss.replace('PROH', '{\\Proh}')
...
def csv_to_latex_commands(csv_file_path, output_file_path, output2_file_path):
    ...
    with open(output_file_path, 'w', encoding='utf-8') as output_file:
        ...
        for index, line in df.iterrows():
            ...
            morph_analysis = replace_reduplication(convert_ipa(remove_exception(str(line.iloc[2]))))  # Morpheme breakdown (e.g., 'ttoen=a	ngas~nges=att=a	gänya=n	llamda=Kurupel=bäne')

            gloss = convert_leipzig(replace_reduplication(str(line.iloc[3])))  # Gloss (e.g., 'story={\Nom} {\Inf}~do={\Abl}={\Nom}...')
